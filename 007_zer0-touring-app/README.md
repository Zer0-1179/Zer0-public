...
### 13. URL短縮 + OGP（DynamoDB + Lambda HTML レスポンス）
...
| 2026-07-03 | v2.3       | **緊急障害対応：Open-Meteo API劣化によるコース生成504を修正**: 目的地天気取得（`fetch_dest_weather`）でTLSハンドシェイクが5秒タイムアウトし、複数コース分累積してLambda Timeout(30秒)を超過・応答なしの504になっていた。`enrich_course`/`geocode_and_filter_spots`に残り時間チェックを追加し、時間不足時は以降の外部API呼び出しをスキップしてAI推定値のまま応答を返すよう修正（外部API劣化時でも必ず応答が返るようになった）。フロントの現在地天気・7日間予報・手動出発地検索の3箇所のfetchにもタイムアウト（5秒）を追加し、Open-Meteo無応答時にUIがハングする問題も解消 |
...
| 2026-07-03 | v2.6       | **ジオコーディングDynamoDBキャッシュ**: `nominatim_geocode`の結果を既存`zer0-touring-ratelimit`テーブルに`geocode#{originバケット}#{地名}`キーでキャッシュ（TTL 90日）。同一地域からの繰り返し利用で目的地・スポット名の重複が多いため、Nominatimの1req/秒直列呼び出しを大幅に削減できる見込み。本番でキャッシュ書き込みを確認済み |
| 2026-07-03 | v2.7       | **コース履歴保存機能**: 1日3回の生成上限で消えていた候補コースをあとから見返せるように、新規`zer0-touring-history`テーブル（端末ID+時刻の複合キー、TTL180日）に生成結果を自動保存。端末ID（`crypto.randomUUID()`）はlocalStorageに保存しリクエストヘッダー`X-Device-Id`で送信。`GET/POST /api/history`ルート追加、トップ画面に「過去のコースを見る」ボタン・履歴一覧画面を追加。本番でsave/list/バリデーション全て検証済み |
...
| 2026-07-03 | v2.9       | **ルートマップ機能**: 詳細画面にLeaflet+OpenStreetMapタイルで現在地・目的地・立ち寄りスポットのマーカーとOSRM実道路ルートのポリラインを表示。leafletをnpm依存に追加しViteでバンドル（デフォルトマーカーアイコンのパス解決が壊れるバンドラー既知問題に対処済み）。CSPに`router.project-osrm.org`（connect-src）・`*.tile.openstreetmap.org`（img-src）を追加。本番でCSPヘッダー・アセット配信・OSRM疎通を確認済み（画面上での目視確認は未実施） |
| 2026-07-03 | v3.0       | **二段階レスポンス化**: `/api/suggest`から距離・スポット座標・目的地天気の取得（Nominatim/OSRM/Google Maps/Open-Meteo）を分離し、Bedrock生成結果をAI推定値のまま即座に返すよう変更（**28秒→9.8秒に短縮、本番実測**）。精密なデータは新設`POST /api/enrich`で詳細画面を開いたコースのみ個別に取得（本番実測5.4秒）。取得できたら詳細画面を自動再描画。DAILY_LIMITの対象は生成のみで、enrichは対象外 |
| 2026-07-03 | v3.1       | **テスト整備**: バックエンドに`backend/tests/`（pytest、boto3モック、17件）を追加し edge-secret検証・XFF末尾信頼・ジオコーディングキャッシュ・履歴保存・二段階レスポンス分離を回帰テスト化。フロントは`fmtHours`/`escHtml`/`getWeatherMeta`を`src/scripts/course-utils.ts`に切り出しVitestで12件のユニットテストを追加（フルリファクタリングはリスク優先で見送り、テスト可能性のみ確保） |
