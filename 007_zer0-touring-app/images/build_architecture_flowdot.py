#!/usr/bin/env python3
"""
構成図(draw.io「プラグイン様式」)から、通信の流れが見える版のSVGを作る（007プロジェクト専用・自己完結版）。

draw.ioで 007_architecture_plugin.drawio を編集 → SVGエクスポート
(同じフォルダに 007_architecture_plugin.drawio.svg として上書き保存)した後、
このファイルと同じフォルダ内で実行すると 007_architecture_plugin_flowdot.svg が
...
HOPS = [
            ["e-users-cloudfront"],
            ["e-cloudfront-s3", "e-cloudfront-apigw"],
            ["e-apigw-lambda"],
            ["e-lambdasuggest-ggeocode", "e-lambdasuggest-groutes"],
            ["e-lambdasuggest-bedrock"],
]
AUX = [
            "e-acm-cloudfront", "e-lambdasuggest-ssm", "e-lambdasuggest-cwlogs",
            "e-lambdasuggest-dynamodb", "e-eventbridge-lambdastats", "e-lambdastats-s3",
            "e-lambdasuggest-nominatim", "e-lambdasuggest-osrm",
        ]
