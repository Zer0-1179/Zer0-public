...
### 「奥多摩湖」がGoogleマップで見つからないとの報告を調査（対応不要と判明）

- ユーザーがGoogleマップで目的地「奥多摩湖」を検索した際に「見つかりません」と表示されたスクリーンショットの提供を受け調査
- 本番`/api/enrich`を直接呼び出して検証した結果、Nominatimジオコーディングは正しい座標（緯度35.7774299・経度139.0102062）を返しており、アプリの`buildMapUrl`もこの座標をそのまま目的地として渡すため、アプリ自身のGoogleマップ連携ボタンは問題なく機能することを確認
- 報告の事象はGoogleマップ側で地名をテキストとして直接検索した際の挙動と推測されるが、アプリ側の座標データ・連携ロジックに不具合は見つからなかった
