...
- ユーザーから「🛣 115kmのように数値だけ書いてあると分かりづらい」との指摘を受け、一覧カード・詳細画面の両方から「🛣 距離km」表示を削除。難易度バッジ（色・アイコン）で概ねの距離感は分かるため、目的地名（🏁）のみを残すシンプルな表示に変更
- 実測距離が目標帯から外れた場合の警告文言「実測距離が目標帯を外れました。別コースをおすすめします。」について、「別コースをおすすめする、は不要では」との指摘を受け「実測距離は目標帯と異なります。」のみに簡略化（事実を伝えるのみとし、行動の提案は削除）
...
- 100_Ritsuan配下プロジェクトの構成図調査を機に001〜008・010を横断確認したところ、007の本番公開SVG（004_portfolioの`src/public/images/007_architecture.svg`）に`light-dark(`が23箇所残存し、2026-09-07に修正したはずのダークモード表示崩れバグ（そもそも007で最初に見つかったバグ）が再発していたことが判明した
- 原因は`build_architecture_flowdot.py`のプロジェクトローカル版（`007_Zer0_TouringApp/images/`配下、他プロジェクトへコピーして使う自己完結型）の`force_light_mode()`が、2026-09-07の`_strip_light_dark()`強化を一度も取り込んでおらず古いロジックのままだったこと。後日の構成図再編集時にこの古いローカル版で再生成し、修正が上書きされたとみられる
- `_strip_light_dark()`を直接適用して`007_architecture_plugin_flowdot.svg`・004_portfolio側の公開コピー双方の`light-dark(`を除去（23箇所→0）。ローカル版スクリプトの`force_light_mode()`自体も最新ロジックへ更新し再発を防止した。同時に他6プロジェクト（001・002・003・005・006・008）のローカル版スクリプトも同じ古いロジックのままだったため、実害はなかったが予防的に同期した
- `bash 004_portfolio/scripts/deploy.sh`で本番デプロイ後、本番URLから画像を取得しローカルとのMD5一致・`light-dark(`残存数0件を確認済み
