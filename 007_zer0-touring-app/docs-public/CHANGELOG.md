...
- **対策3（保険）**: 上記の事前チェックをすり抜けて実測距離が帯域から外れた場合も、実測値が別の帯域（例: 上級）に収まるならその難易度へ正しく再分類して表示するよう修正（従来は「距離条件外」という紛らわしい表示のまま初級ラベルが残っていた）
...
- **既知の限界**: (1) アンカー方式でも稀にBedrockが指示を無視する（本番検証で12回中1回、最終試行でも距離感が乖離のメトリクスが発火）。この場合もenrich時の実測再分類フォールバックが最終的な保険として機能する。(2) Nominatimの1req/秒制限・Bedrock1回約9秒という制約上、Lambda30秒以内に収められるリトライは最大2回までであり、根本的な無制限リトライはできない
