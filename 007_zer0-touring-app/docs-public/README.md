...
| コース内容 | 初級（往復20〜70km）・中級（往復80〜150km）・上級（往復160〜250km） + タグ・立ち寄りスポット（経路順）・帰路提案 |
...
Landing（コースを探す）
  ...
  └─▶ Loading（GPS/出発地取得中 → 天気確認中 → AI生成中）
        └─▶ コース一覧（スワイプカード / 1枚ずつ表示・横スワイプで切り替え）
              │  各カード: 初級🟢（往復20〜70km）/中級🔵（往復80〜150km）/上級🟣（往復160〜250km）
...
行き経由地（`outbound_spots`）と帰り立ち寄り（`return_spots`）を分離し、目的地を含めた全スポットをコース間でも重複排除する。端末内には直近18件だけを保存し、次回のAI提案ではデータ境界付きの除外条件として渡すことで、同じ場所の反復も抑える。AI出力は初級・中級・上級の3件すべてが距離帯・必須スポット・重複なしの条件を満たす場合だけ返す。各コースは観光地または展望台を含み、3コース全体では道の駅を1件以上、温泉・日帰り温泉・銭湯を1件以上必須とする。
