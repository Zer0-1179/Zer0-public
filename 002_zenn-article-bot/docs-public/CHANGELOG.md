...
- `output/`配下の記事番号（`NNN_`プレフィックス）で011・012・013が2件ずつ存在する重複を発見。原因は`scripts/download_article.sh`の採番ロジックが「`output/`内の既存`NNN_*`ディレクトリの**個数**+1」を使っており、過去に一時的にディレクトリ数が実際の最大番号より少ない状態（初期のテストフォルダ削除等）を経由したことで、既に使用済みの番号を再度採番してしまっていたこと
- 各記事の生成日時（フォルダ名内のタイムスタンプ、Lambda生成時刻とほぼ一致）を突き合わせて時系列を確認したところ、重複していた古い5件（4/23 kinesis・4/30 lambda・5/7 sagemaker・5/14 rekognition・5/20 dynamodb）が実際には最も古い記事であり、006以降は時系列通りに正しく並んでいたことを確認。該当5フォルダを`001`〜`005`へリネームし、001〜013まで重複・欠番なしの連番に整理した（記事本文・画像ファイル名はNNNプレフィックスを含まない相対パス参照のため、ディレクトリ名変更のみで内容への影響なし）
- `download_article.sh`の採番ロジックを「ディレクトリ**個数**+1」から「既存`NNN_*`の**最大値**+1」に修正し、同種の再発を防止
- Lambda側にも記事番号用のSSMカウンター（`/zenn-article-bot/article-counter`）が別途存在するが、S3キー生成には使われておらず（`s3_folder`はタイムスタンプのみ）`output/`のNNN番号とは無関係と判明。現状`output/`側の番号割当はダウンロード時にローカルで完結する設計のままとし、SSMカウンター統合は行わなかった（実害のない死コードのため対応不要と判断）
...
### 記事プロンプトに残っていた画像配置マーカー指示を削除（フォークの実装漏れを修正）

- 上記の構成図PNG自動生成廃止作業で見落とし: 記事生成プロンプト内に「はじめに直後」「ハンズオン冒頭」へ`{DIAGRAM_1}`/`{DIAGRAM_2}`マーカーを固定挿入させる指示・直前の「動機づけ文」指示がそのまま残っていた（PNG自体は生成されないため最終的にマーカーは除去されるが、除去後に「構成図から先に見ておきましょう」等の文だけが宙に浮いて残る本文品質バグがあった）
- プロンプトからマーカー・動機づけ文の指示を削除し、画像の配置判断を完全にGPT側（人間の手動ワークフロー）に委ねる設計に統一。マーカー実処理（`_embed_image_placeholders`・`_make_image_placeholder`）も、非空png_paths時の埋め込み・見出しフォールバック挿入ロジックを削除し、LLMが誤って出力した場合の除去のみを行う最小実装に整理（005と同じ設計に統一）
- テスト2件を新仕様（画像は埋め込まずマーカーのみ除去）に更新、全23件パス。再デプロイ後にdry_run実行し、CloudWatch LogsにDIAGRAMマーカー除去の警告が出ないこと（=そもそもLLMがマーカーを出力しなくなったこと）を確認

### 記事系画像生成の全社横断監査（他に残り物がないかの最終確認）

- ワークスペース全体（001〜009・100_Ritsuan配下・110_work_research）を対象に、PIL/matplotlib importを持つPythonファイルを機械的に洗い出して精査
- 001・003・006・007・008・110_work_researchの`scripts/generate_diagram.py`等は「各プロジェクト自身のREADME用アーキテクチャ図生成ツール」であり、記事内画像生成とは無関係と判断（多くは2026-09-03のdraw.io移行で既に未使用化済みの別件）。X投稿Bot（001・003）はそもそも画像投稿機能を持たない
- `render_terminal.py`（002・005、ハンズオン検証のターミナル風スクリーンショット生成）は実際にAWSで実行したコマンド・出力の証跡であり、AI生成イラストと性質が異なるためGPTワークフローの対象外とし、現状維持でユーザー確認済み（2026-09-08）
- Lambda Layerはアカウント全体（ap-northeast-1）で0件（`aws lambda list-layers`で確認）。今回の対応漏れは002のプロンプトマーカー指示のみで、他プロジェクトに記事系画像生成の残り物なしと結論
