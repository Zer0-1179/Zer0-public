...
- 実際の運用（`~/Zer0`をカレントディレクトリにしてプロジェクト相対パスでスクリプトを実行する）に合わせ、コマンドを`cd ~/Zer0 && bash 002_Zenn_Auto_Article_Bot/scripts/download_article.sh`に変更した（従来は`bash ~/Zer0/002_Zenn_Auto_Article_Bot/scripts/download_article.sh`とカレントディレクトリに依存しない形だったが、実際の使い方と表記を一致させた）
