...
#### 生成記事のFableレビューを受けたプロンプト強化

- 実際に生成された記事（vpcトピック）をFableでレビューし、動かないAWS CLIコマンド・手順の論理破綻・技術的事実誤認・セキュリティ上望ましくない記述など11件を発見・修正（AWS公式ドキュメントで裏取り済み）
- 再発防止のためプロンプトにガードレールを追加: OS/AMIの鮮度（Amazon Linux 2はEOL済み）・AWS CLIオプション名の誤りパターン・リソース依存関係と削除順序・NATゲートウェイ料金の実勢値・ALBとAuto Scalingの役割混同防止
- Lambdaランタイムの推奨表記を`python3.13`→`python3.14`に更新（他プロジェクトの最新版と統一）

#### 構成図のクラスター枠クリップバグを修正

- vpcトピックの構成図①で右端のクラスター枠がクリップされる不具合を発見。原因は`_draw_diagram()`のクラスター重なり解消ロジックが枠を押し出した結果、固定xlimの境界に達していたこと
- 個別座標の手直しではなく`_draw_diagram()`自体にxlim/figsize自動拡張ロジックを追加して構造的に修正。全56個の構成図関数で回帰がないことを確認済み
