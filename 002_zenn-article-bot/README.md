...
| 2026-06-27 | v2.1       | IAM最小権限化（コードレビュー反映）：未使用の `ses:SendRawEmail`・本番未使用の Opus モデル ARN（`anthropic.claude-opus-4-8`）・未使用の `aws-marketplace:ViewSubscriptions`/`Subscribe`（`Resource:"*"`）を IAM ポリシーから削除し、Bedrock 権限を実使用の Haiku（foundation-model + inference-profile）のみに限定。テンプレート Description の "Claude Opus" 誤記を "Claude Haiku" に修正。CFnスタック再デプロイ済み |
| 2026-07-01 | v2.2       | diagram_generator.py：`try/finally` で `plt.close(fig)` を保証（figure リーク防止）・`plt.tight_layout()` 削除（bbox_inches='tight' 競合解消）。lambda_function.py：孤立 `{DIAGRAM_N}` マーカークリーンアップ追加（図生成失敗時の残留マーカーを除去）。両修正とも 005 の Opus 4.8 コードレビュー結果を 002 にも反映 |
| 2026-07-03 | v2.3       | **第2巡Fableレビュー HIGH2件修正**: BedrockのIAM ARNがap-northeast-1のみでap-northeast-3ルーティング時AccessDeniedになり得た問題を2リージョン明示で修正。Bedrock応答content空時のIndexError未捕捉を安全抽出関数で修正（トピック選定のmax_tokensも10→20に拡大）。MEDIUM: `RECENT_TOPICS_LIMIT=28`が全トピック数と同値で重複除外が恒久ロックする不具合を20に修正（本番実行で全修正を検証済み） |
| 2026-07-03 | v2.4       | **構成図6トピック追加**: v2.0（2026-06-18）で追加した6サブトピック（ec2_ssm/ec2_ebs/lambda_layers/s3_lifecycle/vpc_endpoint/cloudwatch_insights）が構成図未対応で画像なしのまま記事生成されていた問題を修正。各2枚・計12枚の構成図関数を追加し、SSM/EBS/PrivateLink/S3 Glacierの公式アイコンを新規調達。Lambda ZIPに未同梱の`images/AWS-icon/`に依存しないよう新アイコンは`aws_icons/`へローカル同梱。本番Lambda実行でPNG生成（2枚）を確認済み |
