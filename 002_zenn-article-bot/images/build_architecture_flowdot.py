#!/usr/bin/env python3
"""
構成図(draw.io「プラグイン様式」)から、通信の流れが見える版のSVGを作る（002プロジェクト専用・自己完結版）。

draw.ioで 002_architecture_plugin.drawio を編集 → SVGエクスポート
(同じフォルダに 002_architecture_plugin.drawio.svg として上書き保存)した後、
このファイルと同じフォルダ内で実行すると 002_architecture_plugin_flowdot.svg が
