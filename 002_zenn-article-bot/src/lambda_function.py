...
## 記事の見出し構成（サービスの特性に合わせて自由に決めてよい）

「はじめに」と「まとめ」は必ず含める。その他のセクションは、サービスの特性・読者の理解フローに合わせて
見出しの順番・タイトル・数を自由に変えてよい（毎回同じ構成にしなくてよい）。

### ## はじめに（150〜200文字）
- **冒頭の1〜2文で {topic_name} が「何をするサービスか」を一言で述べる**（サービス名を明示すること）
- なぜこのサービスを学ぶべきかの動機づけ・この記事で学べることの概要
...
### 本文（サービスの特性に合わせて自由に構成する）

以下の要素から適切なものを選び、{topic_name} に合った順序・構成で書くこと（全て含める必要はなく、順番も自由）:

- **サービス概要**（役割・特徴・ユースケース 2〜3例をテーブルで。`:::message` で「一言まとめ」）
- **料金体系**（Free Tier・主要課金項目をテーブルで。`:::message alert` でコスト注意ポイント）
- **ハンズオン**（AWSコンソール操作。ステップ番号付き・動作確認・後片付けを含む）
- **アーキテクチャ例 / 連携パターン**（よく使われる他サービスとの組み合わせ）
- **ベストプラクティス / よくあるミス**（実務向け注意点）
- **応用設定**（`:::details` を活用した補足情報）

ハンズオンを含める場合、セクション冒頭（手順説明の前）に必ず以下を挿入すること（前後に空行必須）:
...
ハンズオンには以下を必ず含める:
- **前提条件**（必要なもの。箇条書き）
- **操作手順**（AWSコンソールの具体的な操作。各ステップにつまずきポイントの注意書き）
- **動作確認**（確認コマンドまたはブラウザ操作・成功時のレスポンス例をコードブロックで必ず示す）
...
記事内では**必ず現在の正式名称**を使うこと。改名がない場合はこのブロックは不要。
