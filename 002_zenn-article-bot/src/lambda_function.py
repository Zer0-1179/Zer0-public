...
- **後片付け**（料金が発生しないようリソースを削除する手順）。**後片付けのステップより後に新規リソースを作成するコードを置かない**（読者が「後片付け済み」と誤解したまま追加コードを実行し、リソースが残留する事故になるため）。片付け対象が複数ステップにまたがる場合は、最後の後片付けステップで一括削除する

**AWS CLIを使うハンズオンの実行環境はAWS CloudShellに統一する**（ローカル環境へのインストール手順は書かない）:
- 前提条件には「AWSマネジメントコンソールにログイン済み（AWS CloudShellを使用）」のみでよい。AWS CLI・Python/pip・Node.js/npm・jq・zip/unzipはCloudShellにプリインストール済み
