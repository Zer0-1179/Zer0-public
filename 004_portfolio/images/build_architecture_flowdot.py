#!/usr/bin/env python3
"""
構成図(draw.io「プラグイン様式」)から、通信の流れが見える版のSVGを作る（004プロジェクト専用・自己完結版）。

draw.ioで 004_architecture_plugin.drawio を編集 → SVGエクスポート
(同じフォルダに 004_architecture_plugin.drawio.svg として上書き保存)した後、
このファイルと同じフォルダ内で実行すると 004_architecture_plugin_flowdot.svg が
...
def force_light_mode(data):
    ...
    リテラルへ直接置換する方式に強化した（中央スクリプトと同一ロジック）。
    2026-09-22判明: この強化版が004ローカル版には反映されておらず、本番公開SVGに
    `light-dark()`が19箇所残存しダークモード表示が壊れたまま配信され続けていた
    （詳細はメモリの`feedback_flowdot_script_drift`参照）。"""
