# 変更履歴（2026-07-13〜2026-09-22）

2026-07-13から2026-09-22までの変更履歴。
...
- 010・011（`100_Ritsuan`配下）の構成図修正作業の一環で全プロジェクトを横断調査したところ、004の本番公開SVG（`src/public/images/004_architecture.svg`）に`light-dark(`が19箇所残存し、2026-09-07に一度修正したはずのダークモード表示崩れバグが再発していたことが判明した。ダークモード環境の閲覧者には、この期間中ずっと配色が崩れて見えていたことになる
- 原因は`build_architecture_flowdot.py`のプロジェクトローカル版（`004_portfolio/images/`配下、他プロジェクトへコピーして使う自己完結型）の`force_light_mode()`が、2026-09-07の`_strip_light_dark()`強化を一度も取り込んでおらず古いロジックのままだったこと。2026-09-10に構成図を再編集・再生成した際、この古いローカル版が使われて修正が上書きされたとみられる
- `_strip_light_dark()`を直接適用して`004_architecture_plugin_flowdot.svg`・`src/public/images/004_architecture.svg`双方の`light-dark(`を除去（19箇所→0）。ローカル版スクリプトの`force_light_mode()`自体も最新ロジックへ更新し、再発を防止した
- CDPでダークモード強制表示にしても常にライト配色で表示されることをヘッドレスブラウザで確認後、`bash scripts/deploy.sh`で本番デプロイ。本番URLから画像を取得しローカルとのMD5一致・`light-dark(`残存数0件を確認済み
