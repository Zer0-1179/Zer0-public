...
### 001〜008の新しい構成図をポートフォリオ表示用に同期・本番デプロイ
...
- 本CHANGELOG.mdで「2026-08-02」のエントリが先頭に誤挿入されていた（プロジェクト共通ルールでは末尾追記）のを、正しい時系列位置に移動
...
### サイト全体のユーザビリティ調査・改善（ユーザー指示「他にカスタマイズしたほうがいいところ調査して」）

- 全`.astro`ファイル（pages/ja・pages/en・components・layouts）を調査し、優先度高4件・中3件・大掛かり枠3件の改善候補リストを作成。うち優先度高の4件を実装
- **ナビの現在地表示**: `Nav.astro`に`isActiveNav()`を追加し、現在のページに対応するナビ項目をアクセント色＋`aria-current="page"`でハイライト（ホームは完全一致、それ以外はプロジェクト詳細等の配下ページも含めて`startsWith`判定）
- **フォーカスリング**: `global.css`に`:focus-visible`でアクセントカラーのアウトラインを追加。既存の`transition-colors`がoutline-colorにも波及しフェード表示になっていたため`transition: none`で即時表示に修正
- **スキップリンク**: `BaseLayout.astro`の`<body>`直後に「メインコンテンツへスキップ」の隠しリンクを追加、`<main>`に`id="main-content" tabindex="-1"`を付与しフォーカス移動先として機能させた
...
### 構成図拡大表示SVGで④Lambda→RSSのフロードットが流れない不具合を修正

- ユーザーから「④のLambdaからRSSまではドットが流れていないけど正常？」と指摘。調査の結果、正常ではなく抜け漏れだった
