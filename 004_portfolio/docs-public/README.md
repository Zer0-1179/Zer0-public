...
![アーキテクチャ図](../images/004_architecture_plugin_flowdot.svg)
...
- 100_Ritsuan配下プロジェクトの構成図調査を機に横断確認したところ、004の本番公開SVGに`light-dark(`が19箇所残存し、2026-09-07に修正したはずのダークモード表示崩れが再発していたことが判明（原因: プロジェクトローカル版スクリプトが2026-09-07の修正を取り込んでいなかった）
- `light-dark()`除去を再適用し、ローカル版スクリプト自体も最新ロジックへ更新した上で本番デプロイ。ダークモード強制表示でのヘッドレスブラウザ確認・本番URLとのMD5一致を確認済み。詳細は[CHANGELOG.md](./CHANGELOG.md)参照
