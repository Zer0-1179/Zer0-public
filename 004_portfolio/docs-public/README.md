...
- ja/en両方のプロジェクト詳細ページの「全体構成図」に適用。デプロイ後、本番ビルド限定の不具合2件（CSP nonce未付与でブロック／nonce付与でTypeScript変換バイパスにより構文エラー）が発覚しユーザー報告を受けて修正。詳細は[CHANGELOG.md](./CHANGELOG.md)参照
- 修正後、production buildをローカル起動しヘッドレスChromiumで拡大・ズーム・Escクローズとコンソールエラー無しを確認してから再デプロイ、本番HTMLで反映を確認
