...
- 007バイクツーリングPWAの集計バッチ(`zer0-touring-stats`)が出力する`stats.json`を表示する`TouringStatsChart.astro`を追加
- touring-appページで累計呼び出し回数と日別バーチャート(直近90日)・表形式の詳細を表示。dataviz skillのガイドラインに準拠、日本語/英語両対応
- stats.json取得失敗時（007側未デプロイ・障害時等）はセクションごと非表示にする設計で、006 CryptoBot実績ページと同様ページ全体は壊れない
