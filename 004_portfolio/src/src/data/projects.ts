...
export const projects: Project[] = [
  {
    ...
    descJa: 'AWSの最新記事をRSSで取得し、Bedrockで口調を変換してXに毎日1回（夜20時）自動投稿するサーバーレスBot。',
    descEn: 'Serverless bot that fetches AWS news via RSS, converts tone with Bedrock, and auto-posts to X once daily at 20:00 JST.',
    longDescJa: '複数のRSSフィード（AWS公式・クラスメソッド・Zenn・Qiita）から14日以内の記事を取得し、段階的フィルタで重複を排除。Amazon Bedrockで @Zer0_Infra の口調に変換し、毎夜20時にURL付きニュース紹介系の投稿をXに自動送信。動的ハッシュタグ生成など、品質を保つための仕組みを実装。',
    longDescEn: 'Fetches articles from multiple RSS feeds (AWS official, Classmethod, Zenn, Qiita) within 14 days, removes duplicates with staged filters. Converts tone to match @Zer0_Infra style using Amazon Bedrock and auto-posts to X every evening at 20:00 JST with URL-attached news intro format. Implements dynamic hashtag generation to maintain content quality.',
    pointsJa: [
      '4つのRSSフィードを並列取得し、14日以内・未投稿の記事のみを段階的フィルタで絞り込み',
      'URL付きニュース紹介系（160文字以内）に特化したプロンプトで高品質な夜投稿を毎日自動生成',
    ...
    pointsEn: [
      ...
      'Evening-only posting with URL-attached news intro style (160 chars max) for consistent daily quality',
  ...
  },
  {
    ...
    descJa: 'AI活用術・会社員あるある系コンテンツを7カテゴリでローテーションしながら毎日自動投稿するBot。',
    descEn: 'Bot that auto-posts AI usage tips and office worker content daily across 7 rotating categories.',
    longDescJa: '仕事×AI・副業・実録・問いかけ・数字・記事感想・Googleトレンド連動の7カテゴリを直近7投稿で重複しないよう管理。水曜はquestion固定、火・金はZenn/QiitaのAI記事感想をURL付きで投稿、日曜はGoogle Trendsと連動。カテゴリ別プロンプト設計で一貫した世界観を表現。',
    longDescEn: 'Manages 7 categories (Work×AI, Side hustle, Real stories, Questions, List posts, Article reactions, Google Trends) without repeating within 7 posts. Wed: question fixed; Tue/Fri: AI article reactions with URLs; Sun: Google Trends-linked posts. Category-specific prompts maintain consistent voice.',
    pointsJa: [
      '7カテゴリの投稿履歴をSSMで管理し、直近7件で同カテゴリが連続しないようローテーション',
    ...
    pointsEn: [
      'Tracks 7-category history in SSM to prevent the same category appearing in 7 consecutive posts',
  ...
  },
  {
    ...
    pointsJa: [
      ...
      '証拠金維持率を4時間毎に監視し、危険水準で自動成行決済・24h未約定キャンセル・注文存在リトライで無人運用を実現',
    ],
    pointsEn: [
      ...
      'Monitors margin ratio every 4h with auto emergency-close; 24h cancel, order-verify retry, and SSM state enable unattended operation',
    ],
