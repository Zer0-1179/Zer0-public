...
| 2026-07-03 | v3.2       | **CSP nonce機構を実装**: 生成していたnonceを誰も消費せずハッシュのみに依存していた問題を修正。全6箇所のインラインスクリプト（Nav/BaseLayout/templates系4ページ）に`nonce={Astro.locals.cspNonce}`を付与し、`middleware.ts`のCSPをnonceベースに簡素化（ハッシュ手動再計算が不要に）。本番でnonce一致を実機検証済み |
