...
FUNCTION_NAME="x-poster-zer0-infra"
STACK_NAME="x-poster-zer0-infra"
REGION="${AWS_DEFAULT_REGION:-ap-northeast-1}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SYNC_SCRIPT="/root/Zer0/sync_to_public.sh"
MODE="${1:-}"

echo "=== X Poster Bot (zer0-infra) デプロイ ==="
...
if [ "$MODE" = "--full" ]; then
    ...
    echo ""
    echo "=== X Poster Bot (zer0-infra) フルデプロイ完了 ==="
...
echo ""
echo "=== X Poster Bot (zer0-infra) コード更新完了 ==="
echo "DRY RUNテスト: ./deploy.sh --test"
