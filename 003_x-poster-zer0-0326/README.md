...
| 2026-06-22 | v2.0       | バグ修正＋ブラッシュアップ：url_reactionハッシュタグ二重付与防止（モデル挿入タグを除去してから pick_hashtag）、url_reaction を NO_HASHTAG_RATE 除外（専用タグを意図通り1個付与）、FORCE_CATEGORY=url_reaction フォールバック時の category 未セットを修正、hashtag_re をモジュール定数に変更、CFn に `ReservedConcurrentExecutions: 1` 追加、モジュール docstring を1行に短縮 |
