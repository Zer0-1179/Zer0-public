#!/usr/bin/env python3
"""
構成図(draw.io「プラグイン様式」)から、通信の流れが見える版のSVGを作る（003プロジェクト専用・自己完結版）。

draw.ioで 003_architecture_plugin.drawio を編集 → SVGエクスポート
(同じフォルダに 003_architecture_plugin.drawio.svg として上書き保存)した後、
このファイルと同じフォルダ内で実行すると 003_architecture_plugin_flowdot.svg が
...
AUX = ["e-lambda-ssm", "e-lambda-sns", "e-sns-email"]
