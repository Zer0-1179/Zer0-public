...
echo "=========================================="
echo "  X Poster Bot (zer0-0326) Deploy (no S3)"
...
aws cloudformation deploy \
    --template-file cfn-x-poster-zer0-0326.yaml \
...
echo ""
echo "  Test commands:"
echo "  # ランダムカテゴリ投稿（月・木20:00と同じ）"
echo "  aws lambda invoke --function-name x-poster-zer0-0326 --region ${REGION} \\"
echo "    --payload '{\"mode\": \"random\"}' --cli-binary-format raw-in-base64-out response.json"
echo ""
echo "  # トレンド投稿（日曜10:00と同じ）"
echo "  aws lambda invoke --function-name x-poster-zer0-0326 --region ${REGION} \\"
echo "    --payload '{\"mode\": \"trend\"}' --cli-binary-format raw-in-base64-out response.json"
echo ""
echo "  # DRY RUNテスト（X投稿なし）: bash scripts/test_invoke.sh"
echo ""
echo "  Logs:"
echo "  aws logs tail /aws/lambda/x-poster-zer0-0326 --since 5m --region ${REGION}"
echo "=========================================="
