...
def draw():
    ...
    # --- 「自分自身のラベルを踏む」問題への対応(2026-08-09、ユーザー指摘) ---
    # matplotlibのarrowprops shrinkA/shrinkB(=42pt≈0.666データ単位)は、線の
    # 描画開始・終了点をノード中心からその分だけ後退させるだけで、ノード自身の
    # ラベル(アイコン下端からさらに0.2下、2行ラベルなら高さ約0.34)までは
    # 避けてくれない。そのため「そのノードから真下/真上に一直線で伸びる経路」は
    # 出発・到着のたびに自分自身のラベル文字を貫通してしまう(cf→s3lpや
    # lambda_fw→ses_outなど、一見きれいな直線ほど陥りやすい)。
    # get_window_extent実測+shrink後の実描画区間を検証するスクリプトで全経路を
    # 再チェックし、該当箇所には出発・到着の直後だけ小さく迂回する経由点を追加した。
    edges = [
        ('eb',        'lambda',   ''),
        ('lambda',    'site',     'GET収集'),
        ('lambda',    'ssm',      '設定取得'),
        ('lambda',    'cw',       ''),
        # lambdaの真下(x=6.0)はlambda自身のラベルの真ん中を通ってしまうため、
        # 出発直後にx=5.05へ小さく迂回してから経路を下りる。
        ('lambda',    'dlq',      '', [(5.05, 11.4)]),
        ...
        # cf→s3lpはs3lpをcfと同じグリッド列(x=9.5)に置いて一直線に見せたいが、
        # 直下へ出発するとcf自身のラベルを貫通するため、出発直後だけx=11.0へ
        # 小さく迂回してから下りる(ほぼ直線に見えつつラベルを踏まない)。
        ('cf',        's3lp',     '', [(11.0, 18.0)]),
        # cf/acmをregion外へ退避させたことでy=6.5〜7.3が完全に空いたため、
        # browser→apigwは迂回なしの直線で引ける(2026-08-09に単純化)。
        ('browser',   'apigw',    '事前登録\nfetch'),
        ('apigw',     'lambda_wl',''),
        # lambda_wlから真下気味に出発するとlambda_wl自身のラベルにかかるため、
        # 出発直後にx=14.0へ小さく迂回してからddb_wlへ下りる。
        ('lambda_wl', 'ddb_wl',   '', [(14.0, 5.7)]),
        ...
        ('ses_in',    's3mail',   ''),
        ('s3mail',    'lambda_fw',''),
        # lambda_fw→ses_outはses_outと同じグリッド列(x=20.0)で一直線に見せたいが、
        # 到着直前にses_out自身のラベルを貫通するため、到着直前だけx=21.3へ
        # 小さく迂回してから合流する。
        ('lambda_fw', 'ses_out',  '転送', [(21.3, 5.9)]),
        ...
        # まで下りてから右へ進み、lambda_swへ合流する。出発直後(apigw自身の
        # ラベル)と到着直前(lambda_sw自身のラベル)もそれぞれ小さく迂回する。
        ('apigw',     'lambda_sw','', [(10.6, 6.0), (10.3, 1.1), (16.5, 1.1), (17.6, 4.3)]),
        # lambda_swから真下気味に出発するとlambda_sw自身のラベルにかかるため、
        # 出発直後にx=15.5へ小さく迂回してからddb_wlへ下りる。
        ('lambda_sw', 'ddb_wl',   '', [(15.5, 4.5)]),
        ('lambda_sw', 'ses_out',  ''),
    ]
