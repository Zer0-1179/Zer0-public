...
- さらに「矢印はラベル文字の下から出ているように見せてほしい」との明確な指示を受け、`arrowprops`の出発側`shrinkA`を42pt→3ptに縮小。アイコンはzorder=4で線(zorder=3)より前面にあるため、アイコン内側は自動的に隠れる仕組みを利用し、線の可視部分がアイコン直下・ラベル文字を通過する形で開始するよう変更（到着側`shrinkB`は矢頭を明瞭に見せるため42ptを維持）
