...
- 改めて「直線が自ノードのラベル文字を自然に貫通する接続のみ」という条件を確認し、該当する5接続（Lambda→SQS、CloudFront→S3、lp-waitlist Lambda→DynamoDB、stripe-webhook Lambda→DynamoDB、mail-forwarder Lambda→SES送信）だけ、始点/終点をラベル寄りにする対応に変更。判定基準は「中心点どうしを直線で結んだ場合に自分自身のラベル矩形を通過するか」で機械的に特定した
