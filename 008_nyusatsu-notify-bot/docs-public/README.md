...
**現在のステータス: v0.23。パイロット実装・AWS上でテスト運用中（横浜市単体）。LP・法務ページの配色をメールと同じ青系に統一し料金表記・FAQ・フォームをブラッシュアップ、見出しの改行・FAQアコーディオンの開閉挙動も調整。次はStripe Payment Links発行**  
...
| 2026-07-12 | v0.22      | 見出し「毎朝自動でチェック」が単語の途中で不自然に折り返っていた問題をnowrapスパンで修正(ユーザー指摘) |
