...
**現在のステータス: v0.19。パイロット実装・AWS上でテスト運用中（横浜市単体）。Stripe本番アカウント開設・特定商取引法に基づく表記整備に続き、Stripe Webhookで課金者⇔購読者を突合する仕組み（stripe_webhook Lambda）を実装。決済完了で自動active化、collectorは配信ゲート用SSMフラグでいつでも課金必須化に切替可能**  
...
| 日付       | バージョン | 内容                                                                                                                                                           |
| ---------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-07-11 | v0.17      | Stripe本番アカウントを開設。銀行口座・明細書表記・本人確認書類・セキュリティ対策措置状況申告書を提出しアカウント一時停止を解除。IAMユーザーのMFA未設定を発見し是正 |
