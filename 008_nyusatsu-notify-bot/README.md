...
課金開始前にFableで市場調査をやり直した結果（2026-07-08）、法務対応（特商法表記・利用規約・プライバシーポリシー）と流量計測が課金開始の前提条件と判明した。2026-07-09にA群（登録確認・ワンクリック配信停止等）・B-1（実配信接続）・B-3（バウンス/苦情対応）・プロダクト品質レビュー対応（v0.11、参加資格情報・バックフィルウェルカムメール等）が完了。Fableの採点は実装前6/10→実装後8〜9/10相当（法務・決済除く）。
...
3. 流量計測を1〜2ヶ月継続し、価格・対象エリアを最終判断
4. キーワード拡充（消防用設備点検・造園/緑地/街路樹・除草・設備保守点検等）: 「ビルメンテナンス業向け」の看板に対し現行4キーワードは狭すぎるとFableが指摘。SSMパラメータのみで実装可能（コード変更不要）だが、全購読者一律配信のためノイズとのトレードオフを要検討
5. 複数自治体への拡大: 横浜市は公告が週1回のため通知頻度に構造的な上限がある。近隣市を追加すればムラを緩和できるが工数大、流量計測の結果次第
