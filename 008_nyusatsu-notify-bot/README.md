...
**現在のステータス: v0.13。パイロット実装・AWS上でテスト運用中（横浜市単体・課金導入前）。実配信接続・参加資格情報付き通知・バックフィルウェルカムメールまで実装済み。Fable高効度コードレビューで検出した10件の不具合を修正・デプロイ・本番動作確認済み**  
...
7. 購読者管理のマルチテナント化は保留（エリア/業種のフィルタ軸が実際に増える段階になるまで不要というFable判断）
...
| 日付       | バージョン | 内容                                                                                                                                                           |
| ---------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-07-09 | v0.11      | Fable品質レビュー実施。参加資格情報の追加・バックフィルウェルカムメール・週次サマリー購読者配信・LP期待値是正を実装。mail_forwarderの件名文字化けも修正         |
| 2026-07-09 | v0.12      | Fable指摘のキーワード拡充。清掃系4語に消防用設備点検・造園・緑地・街路樹・除草・設備保守点検の6語を追加し計10語に。SSM/CFn更新・動作確認済み                   |
| 2026-07-10 | v0.13      | Fableコードレビュー(高effort)で10件の不具合を検出・修正。SES送信失敗の握りつぶし・配信停止意思の無視・第三者による停止解除・幽霊レコード生成等を解消。CloudWatchアラーム2件追加・deploy_lambdas.sh新設 |
