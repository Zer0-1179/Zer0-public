...
① 相場の状態をチェック（Analyzer）
    ...
    └─ BTCが200EMA以下 → 下落相場 → ショートシグナルを探す
            ↓ シグナル（エントリータイミング）があれば
② 注文を出す・管理する（Executor）
    ├─ 新規: bitbank信用取引に指値注文（ロング or ショート）
    ├─ 約定後: 利確・損切り注文を自動発注
...
- **初期資金**: 30,000円以上を bitbank に入金推奨（3ポジション分の証拠金）
- **APIキー権限**: 「取引」「残高照会」のみ許可。**出金権限は付与しない**
- **信用取引リスク**: 証拠金維持率が一定水準を下回ると強制決済（ロスカット）される
- **日次利息**: 0.04%/日（ロング・ショート共通）が保有期間分発生する
