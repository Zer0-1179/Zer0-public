...
| 2026-07-03 | v4.3       | **Binance APIフォールバックホスト追加**: `fetch_binance`を同一ホスト3回再試行から`api.binance.com`→`api1〜api4.binance.com`→`data-api.binance.vision`（公開データ専用ミラー）へのホストローテーションに変更。単一ホスト障害・レート制限時にシグナル検出が4時間丸ごと欠落するのを防止。本番実行・実Binance APIへの疎通で確認済み |
| 2026-07-03 | v4.4       | **pytestテスト整備＋backtest⇄analyzerパリティテスト追加**: `tests/`（プロジェクトルート直下、deploy.shのzip対象外）にpytest 27件を新設。純関数・state⇄実建玉リコンサイル・セーフモード・Binanceフォールバック・analyze_coinのカバレッジに加え、backtest.py（pandas実装）とanalyzer（純Python実装）のATR/Supertrendが同一の合成データで収束することを検証するパリティテストを追加（先頭30バーのEWM起点差はwarmupとして許容、それ以降は完全一致を要求）。二重実装のドリフトを検知する仕組みが今回初めて整った |
