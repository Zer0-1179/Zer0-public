#!/usr/bin/env python3
"""
構成図(draw.io「プラグイン様式」)から、通信の流れが見える版のSVGを作る（006プロジェクト専用・自己完結版）。

draw.ioで 006_architecture_plugin.drawio を編集 → SVGエクスポート
(同じフォルダに 006_architecture_plugin.drawio.svg として上書き保存)した後、
このファイルと同じフォルダ内で実行すると 006_architecture_plugin_flowdot.svg が
...
NUM = "006"
HOPS = [
            ["e-eventbridge-analyzer", "e-eventbridge-executor", "e-eventbridge-weeklysum"],
            ["e-analyzer-binance"],
            ["e-analyzer-ssm"],
            ["e-executor-ssm"],
            ["e-executor-bitbank"],
...
AUX = ["e-executor-failnotif", "e-failnotif-ses", "e-ses-email", "e-cwalarm-sns", "e-sns-email"]
