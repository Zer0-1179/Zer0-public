...
v3.4変更: 本番の30分サイクルを忠実に再現したモデルで再検証し トレール 0.75→2.0
"""
...
# v3.4: 旧0.75はbacktest.pyのトレール判定バグ（TP1約定と同じ4h足で即決済判定してしまう）
# 由来の誤最適化だったと判明。本番の30分サイクルを忠実に再現したモデルで再検証し、
# 5年PF 1.49→1.82・成長率+53.5%→+108.2%（年次5ブロック全てで改善、ペアード検定p=0.0031、
# 選択手続きごとのウォークフォワード4fold全てでOOS勝ち）を確認して2.0に変更。
TRAIL_MULT  = 2.0   # トレーリング幅 = 極値 ± ATR × 2.0
