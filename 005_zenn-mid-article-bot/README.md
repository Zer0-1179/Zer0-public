...
| 2026-07-02 | v2.4       | v2.3の**AI技術レビュー・自動修正機能を撤回**。レビュー（検出）は実機で一貫して実バグを検出できたが、自動修正（1回のBedrock呼び出しで記事全文を書き直す方式）は指摘された箇所を直しても別の技術的矛盾を新規に作り込むケースを実機で確認（例: 指摘されたgzipエンベロープ問題を修正した際にParquet変換の実装自体が欠落する等）、信頼性がコストに見合わないと判断し撤回。`review_technical_accuracy()`・`fix_article()`・関連のメール文面・CFnの`ReviewModelId`パラメータを削除し、Lambda処理フローをv2.2以前の構成に戻した。**代わりに、記事の複雑さ自体を下げる方向に方針転換**: 全16トピックの使用サービス数を5→3に削減（今回のセッションで見つかった実バグの多くは特定のサービス組み合わせが原因だったため、その組み合わせ自体を除外——log_analyticsはCloudWatch Logs+Firehoseを外して直接CloudTrail→S3、data_lakeはLake Formation+QuickSightを除外、realtime_notifyはEventBridgeを除外、cicd_pipelineはCodeDeploy+ECSブルーグリーンを除外しCodePipeline+CodeBuild+ECRのビルドパイプラインのみに簡素化）。**構成図生成も全面刷新**: トピックごとに手作業でハードコードしていた32個の図関数（本文の`services`リストと食い違うバグの温床だった。今回の調査で`container_platform`など他トピックにも同種の不整合が既存していたことが判明）を全廃し、`services`リストから直接ノードを組み立てる汎用テンプレート関数`_diagram_generic()`1本に統合。図と本文が構造的に一致するため、この種の不整合バグが原理的に起きなくなった。**既知の限界**: サービス数を減らした記事でも、単体サービスの仕様理解ミス（ALBへのAliasレコード未使用、S3レプリケーションFilter構文、バージョニング有効バケットの削除手順等）は実機検証で引き続き確認されており、自動検出の仕組みは無い状態に戻った点は要留意 |
