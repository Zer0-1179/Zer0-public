...
| 2026-06-16 | v1.8: 記事内構築手順をCFnテンプレート→AWS CLI中心に変更。プロンプト改修（変数定義セクション必須・コメント形式・クリーンアップセクション追加等）、CFn `validate_template` 廃止→CLI必須要素チェックに置換、`cfn` boto3クライアント削除 |
| 2026-06-16 | v1.9: コードレビュー指摘6件修正：①bashブロック例の`\`が行継続として除去されるバグ修正②例外ハンドラのCFn表記修正③CLIバリデーターをbashブロック内限定awsチェック＋クリーンアップ同義語対応に改善④両CFnスタックから`cloudformation:ValidateTemplate` IAM権限を削除 |
