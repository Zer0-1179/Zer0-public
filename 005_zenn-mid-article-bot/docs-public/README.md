...
- 2026-08-01の実行事故で生成された8記事（009〜016）をFableでレビューした結果、全8記事でHIGH級指摘（実行不能なコマンド・本文と実装/構成図の不一致・コスト試算の誤り等）が見つかった問題を受け、`ARTICLE_PROMPT_TEMPLATE`に002（2026-07-16実施）と同様のガードレールを追加
- 追加した4点: ①本文で「実現する」「作成する」と明言した機能は対応するCLIコマンドを必ず含める自己整合性チェック指示、②`--attributes`等JSONを受け取るオプションへのshorthand構文でのJSON直接埋め込み禁止（`file://`参照を使う）、③月額コスト目安の計算式明記・日額/月額混同防止・無料枠控除の明示、④構成図は`services`リストから機械的に生成されるため、本文の説明を実際の構成と一致させ図にない分岐を書かない指示
- 全24トピック×3回（計72通り）でプロンプト整形にエラーがないことを確認。ユニットテスト19件パス
- dry_run+test_mode実機実行で新プロンプトでの記事生成（25,424文字）成功を確認（実課金・実メール送信なし）。本番Lambdaへコードデプロイ済み（プロンプト変更のみのためCFnパラメータ変更なし、`update-function-code`で直接反映）
