...
- 002のLambda Layers記事ハンズオンレビューで実行時バグが複数見つかった経緯を踏まえ、005の`ARTICLE_PROMPT_TEMPLATE`にも同様のガードレールを横展開
- `_SECTION_STEPS`をAWS CloudShell標準に更新し、`aws lambda update-function-code`で直接デプロイした
